...
export function detailPage(recipe: Recipe, lang: Lang): string {
  ...
  const navRight = `
    ...
    <button type="button" class="nav-btn" onclick="shareIngredients()" title="${esc(t('detail.share', lang))}">${SHARE_ICON}</button>`;
  ...
  const content = `
    <div class="portion-bar">
      ...
      <button type="button" class="portion-btn" onclick="changePortions(1)">+</button>
      ${recipe.cookingTime ? `<span style="display:flex;align-items:center;gap:4px;font-size:13px;color:var(--text-2);margin-left:8px">${CLOCK_ICON}${esc(formatCookingTime(recipe.cookingTime))}</span>` : ''}
    </div>
